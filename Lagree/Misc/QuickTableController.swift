...
class QuickTableController : UITableViewController {
    ...
    override func tableView(_ tableView: UITableView, viewForHeaderInSection section: Int) -> UIView? {
        ...
        if(section == 0)
        {
            ...
            button.setImage(UIImage(named: "iks"), for: UIControlState.normal)
            button.addTarget(self, action: #selector(QuickTableController.exitModal), for: .touchUpInside)
            headerView.addSubview(button)
        }
        
        //Bottom Border
        let borderBottom = UIView(frame: CGRect(x:0, y:self.sectionHeight-1, width: tableView.bounds.size.width, height: 1.0))
